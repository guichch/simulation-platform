...
import { stationarySatellite } from "../../../../../public/data/stationarySatellite"
import axios from "axios"
import { Lmap } from "../init2dMap"
import L from 'leaflet'
// import "leaflet.markercluster";
// import sate from "../../../../components/OperatePanel/satelliteResources_wxzy_xq.vue"
    ...
    if (sessionStorage.getItem("allSatCollection") == null) { //如果缓存中没有卫星数据的话
        axios({
            ...
            if (res.data.length > 0) {
                searchedSatCollection = res.data
...
let WhereSatCollectionData = []
    //清除卫星点
export const clearSatOn3dMap = () => {
    viewer.dataSources.removeAll()
}

//根据条件查询的卫星
export const getWhereSatCollection = (data, map, type, obj) => {
    if (type != "control") { //control
        WhereSatCollectionData = data
    }

    if (type == "where" || type == "control") {
        WeiXingGuiDao(WhereSatCollectionData, map)
    }
    if (obj == undefined) {
        objWXZY = type
    } else {
...
function getStaMarkerImage(satcname2d, satpos2d, wd2d, searchedSatCollection) {
    //定义卫星图标，显示内容和点击事件等
    var satIcon = L.icon({
        iconUrl: require('../../../images/sat2dmap (2).png'),
        iconSize: [48, 48],
    });
