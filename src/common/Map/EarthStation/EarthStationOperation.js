...
export let addPointEarthStation = (EarthStationData, map, obj) => {
    ...
    EarthStationData.forEach(data => {
        if (data.teleportOwnership) {
            imageURL = "/images/地球站1.png"
        } else if (!data.teleportOwnership) {
            imageURL = "/images/地球站2.png"
        }
        if (map == "3dmap") {
            ...
            marker.on('click', function(e) { //点击地球站点，显示地球站点的信息
                let row = e.target.options.attribute;
                obj.$refs.EarthStation_xq.clickRow = row;
                obj.$refs.EarthStation_xq.earthstation_xqShow = true;
                obj.$refs.satelliteResources_wxzy_xq.satelliteResources_wxzy_xqShow = false
                obj.$refs.EarthStation_xq.down();
            });
            //需要测试
